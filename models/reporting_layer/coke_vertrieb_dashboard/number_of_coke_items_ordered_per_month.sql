select sum(  "order_item".amount )
	 ...
	 , max( concat( extract( month from "order".created_at ) , '-'
	, extract( year from "order".created_at ) ) )  Calender_week
-- into prod_reporting_layer.number_of_coke_items_ordered_per_month
...
where ( product.name like '%ola%'
  ...
  and "order".created_at >= '2022-01-01'::date

group by extract( month from "order".created_at ), extract( year from "order".created_at )
order by extract( year from "order".created_at ) asc, extract( month from "order".created_at ) asc