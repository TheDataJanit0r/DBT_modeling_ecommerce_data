with active_customers_current_week_t_0 as
	(
		...
		where user_uuid is not null and   "Rottkapechen Kunde"= true--"order".created_at <= now( ) - '0 week':: interval
   ...
   , current_week_t_0 as (
	                         ...
	                         SELECT count( distinct id_customer )                                 AS "count"
			                      ...
			                      , max( concat( extract( 'month' from NOW( ) ), '-',
			                                     extract( 'year' from NOW( ) ) ) ) as "Week of the Year"
