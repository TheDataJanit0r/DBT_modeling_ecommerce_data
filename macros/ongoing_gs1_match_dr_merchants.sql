{% macro ongoing_gs1_match_dr_merchants(GFGH_LIST) %}
with exclude as (
...
select
   ...
   k.tool_link
from {{ ref('gs1data') }} g
join {{ref(GFGH)}} k on k.gpu = g.gtin_packaging_unit and k.kollex_active = TRUE and coalesce(k.direct_shop_release, 'false') like '%rue%'
